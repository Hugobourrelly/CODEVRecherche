...
for i in range(n):
    ...
    Module_Fenetree, Xmodf = td.fenetrage(Module[:630], Liste_Frequences[:630], taille_fenetre)
    ...
    """
    params = [max(Module_Fenetree), ad.calculDemiLargeurMiHauteur(list(Module_Fenetree),Xmodf), fres_empirique]

    Listes_Des_Parametre_De_Fit[0].append(params[0])
    Listes_Des_Parametre_De_Fit[1].append(params[1])
    Listes_Des_Parametre_De_Fit[2].append(params[2])

    """Trace et sauvegarde des figures"""
    plt.plot(Liste_Frequences, Module, color='blue')
    plt.plot(Liste_Frequences, module(Liste_Frequences, params[0], params[1], params[2]), color='red')
    plt.title(str(H[i])+" Tesla ; Courant de : "+str(Courant[i])+"A")
    """Répertoire à spécifier : """
    plt.savefig(r"D:\IMT-Atlantique\1Année\CODEV\Courbes\Module_fit_manuel\\" +str(i)+"_"+str(H[i])+"Tesla;Courant_de_"+str(Courant[i])+"A"+".png")
    plt.clf()
